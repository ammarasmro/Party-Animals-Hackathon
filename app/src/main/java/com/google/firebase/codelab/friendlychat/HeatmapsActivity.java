...
public class HeatmapsActivity extends BaseActivity {
    ...
    protected void startDemo() {
        ...
        if(intentThatStartedThisActivity.hasExtra(Intent.EXTRA_TEXT))
            data = intentThatStartedThisActivity.getStringExtra(Intent.EXTRA_TEXT);

        getMap().moveCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(53, -113), 4));
...
//        try {
//            mLists.put(getString(R.string.people), new DataSet(readItems(R.raw.people)));
//        } catch (JSONException e) {
//            Toast.makeText(this, "Problem reading list of markers.", Toast.LENGTH_LONG).show();
//        }

        try {
            mLists.put("People", new DataSet(readItems(data)));
...
//            TextView attribution = ((TextView) findViewById(R.id.attribution));
            ...
            if (mProvider == null) {
                mProvider = new HeatmapTileProvider.Builder().data(
                        mLists.get(getString(R.string.people)).getData()).build();
